 ...
 * 手机日志
 */
public class ScanUser extends Configured implements Tool {
    ...
    private static class MobileMapper extends TableMapper<Text, Text> {
        ...
        protected void map(ImmutableBytesWritable rowKey, Result result, Context context) throws IOException, InterruptedException {
            ...
            String fDownTime = Bytes.toString(result.getValue(Bytes.toBytes("acti"), Bytes.toBytes("f_down_time")));
            String fDownSrc = Bytes.toString(result.getValue(Bytes.toBytes("acti"), Bytes.toBytes("f_down_src")));
            ...
            srcUserBean.setDeviceId(deviceId);
            srcUserBean.setMType(mType);
            srcUserBean.setMMachine(mMachine);
            srcUserBean.setFDownTime(fDownTime);
            srcUserBean.setFDownSrc(fDownSrc);
            srcUserBean.setToken(token);

            try {
                if (!StringUtils.isBlank(deviceId)){
                    context.write(new Text(deviceId), new Text(srcUserBean.rowLine()));
                }else {
                    return;
                }
    ...
    public int run(String[] args) throws Exception {
        try {
            ...
            Job job = Job.getInstance(this.getConf(), ScanUser.class.getSimpleName());
            job.setJarByClass(ScanUser.class);
