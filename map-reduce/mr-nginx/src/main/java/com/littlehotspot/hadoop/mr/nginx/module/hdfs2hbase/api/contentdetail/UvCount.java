 ...
 * 手机日志
 */
public class UvCount extends Configured implements Tool {
    ...
    private static class MobileMapper extends TableMapper<Text, Text> {
        ...
        protected void map(ImmutableBytesWritable rowKey, Result result, Context context) throws IOException, InterruptedException {
            ...
            try {
                ...
                sourceBean.setMobileId(mobileId);
                ...
                if (!StringUtils.isBlank(contentId)){
                    context.write(new Text(contentId+"|"+categoryId+"|"+mobileId+"|"+date), new Text(sourceBean.rowLine3()));
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
        ...
        protected void reduce(Text key, Iterable<Text> value, Context context) throws IOException, InterruptedException {
            try {
                ...
                while (iterator.hasNext()){
                    ...
                    Matcher matcher =CommonVariables.MAPPER_FORMAT_REGEX.matcher(rowLineContent);
                    if (!matcher.find()){
                        return;
                    }
                    bean.setContentId(matcher.group(2));
                    bean.setCategoryId(matcher.group(3));
                    bean.setMobileId(matcher.group(1));
                    bean.setDateTime(matcher.group(6));

                }
                context.write(new Text(bean.getContentId()+"|"+bean.getCategoryId()+"|"+bean.getDateTime()), new Text(bean.rowLine4()));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
        ...
        protected void reduce(Text key, Iterable<Text> value, Context context) throws IOException, InterruptedException {
            try {
                ...
                while (textIterator.hasNext()) {
                    ...
                    bean.setContentId(matcher.group(2));
                    bean.setCategoryId(matcher.group(3));
                    bean.setDateTime(matcher.group(4));
                    count++;
                }
                bean.setUvCount(count.toString());
                context.write(new Text(bean.rowLine5()), new Text());
    ...
    public int run(String[] args) throws Exception {
        try {
            ...
            Job job = Job.getInstance(this.getConf(), UvCount.class.getSimpleName());
            ...
            SingleColumnValueFilter optfilter = new SingleColumnValueFilter(Bytes.toBytes("attr"),
                    Bytes.toBytes("option_type"), CompareFilter.CompareOp.EQUAL,Bytes.toBytes("show"));
            ...
            job.setCombinerClass(UvCount.Combiner.class);
            /**作业输出*/
