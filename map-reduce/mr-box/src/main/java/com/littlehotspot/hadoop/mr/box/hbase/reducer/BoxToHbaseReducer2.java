 ...
 * 整合数据reducer
 */
public class BoxToHbaseReducer2 extends Reducer<Text, Text, Text, Text> {
    private HBaseHelper hBaseHelper;
    @Override
    protected void reduce(Text key, Iterable<Text> value, Context context) throws IOException, InterruptedException {
        try {
            Iterator<Text> textIterator = value.iterator();
            String rowLineContent = textIterator.next().toString();
            ...
            Long timestr=9999999999L-Long.parseLong(StringUtils.left(boxBean.getTimestamps(),10));
            String rowKey=boxBean.getMac()+"|"+timestr;
