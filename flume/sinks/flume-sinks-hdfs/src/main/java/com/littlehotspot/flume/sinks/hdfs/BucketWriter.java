...
import com.littlehotspot.flume.sinks.hdfs.HDFSEventSink.WriterCallback;
...
class BucketWriter {
    ...
    public synchronized void append(final Event event) throws IOException, InterruptedException {
        ...
        if (idleFuture != null) {
            ...
            if (!idleFuture.isDone()) {
                try {
                    ...
                    LOG.warn("Timeout while trying to cancel closing of idle file. Idle" + " file close may have failed", ex);
                } catch (Exception ex) {
        ...
        try {
            ...
            LOG.warn("Caught IOException writing to HDFSWriter ({}). Closing file (" + bucketPath + ") and rethrowing exception.", e.getMessage());
            close(true);
            throw e;
        }
